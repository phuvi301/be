...
const PlaylistsController = {
    ...
    async getPlaylist(req, res) {
        try {
            ...
            let playlistQuery = Playlists.findById(req.params.id)
    ...
    async updatePlaylist(req, res) {
        try {
            const { title, description, thumbnailUrl, tracks, status, isPublic } = req.body || {};
            const updateData = {};

            if (typeof title === "string") updateData.title = title;
            if (typeof description === "string") updateData.description = description;
            if (typeof thumbnailUrl === "string") updateData.thumbnailUrl = thumbnailUrl;
            if (Array.isArray(tracks)) updateData.tracks = tracks;

            if (typeof isPublic === "boolean") {
                updateData.status = isPublic ? "public" : "private";
            } else if (typeof status === "string" && ["public", "private"].includes(status)) {
